@testset "CUSUM" begin
    ...
    @testset "Negative prices cause DomainError (log requirement)" begin
        # CUSUM uses log(prices), so negative prices throw DomainError.
        # Note: log(0.0) returns -Inf (no error), but log(-x) throws.
